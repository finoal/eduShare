...
const PurchaseCard = memo(({ nft, updateCollectible }: { nft: Collectible; updateCollectible?: (updatedNft: Collectible) => void }) => {
  ...
  const handleDownloadSuccess = useCallback(() => {
    // 如果提供了更新函数，则更新UI中的下载次数
    if (updateCollectible && nft) {
      const updatedNft = {
        ...nft,
        downloadCount: (nft.downloadCount || 0) + 1
      };
      updateCollectible(updatedNft);
    }
  }, [nft, updateCollectible]);
      ...
      <div className="card-body p-4 flex flex-col">
        ...
        <div className="mt-2 space-y-2 w-full">
          ...
          {nft.eduResource && (
            <DownloadButton
              url={nft.eduResource}
              fileName={nft.fileName || nft.name || `资源-${nft.id}`}
